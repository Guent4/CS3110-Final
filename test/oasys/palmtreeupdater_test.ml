...
let setup_tree () =
	...
	let config = {repo_dir=dir; current_branch="master"; username=""; 
		password=""; upstream=""} in
	let tree = {head=("", "", []); index=([],[]); work_dir=[]; 
		commit_tree= CommitTree.add "master" [] (CommitTree.empty)} in
	(tree,config)
...
TEST_MODULE "init tests" = struct
	...
	TEST "test feedback" =(
		let x = "Initialized empty OASYS repository in " ^ 
			config'.repo_dir ^ ".oasys/" in
		match feedback with
...
TEST_MODULE "commit tests" = struct
	...
	(
		...
		match master with
		| (i, m, c)::t -> (m = "test") && 
			(List.mem c ((Sys.getcwd()) ^ "/test_proj/"  ^ "wuggle.txt")) && (List.mem c ((Sys.getcwd()) ^ "/test_proj/"  ^ "test.txt"))
		| _ -> false
	)
end
...
TEST_MODULE "reset FILES tests" = struct
	...
	let (tree'',config'',feedback') = update_tree (ADD,[EMPTY],["add1.txt"]) 
		tree' config'
...
TEST_MODULE "reset tests" = struct
	...
	let (tree'',config'',feedback') = update_tree (ADD,[EMPTY],["add1.txt"]) 
		tree' config'
	let (tree''',config''',feedback'') = update_tree (COMMIT,[MSG],["add1.txt"]) 
		tree'' config''
	let (tree4, config4, feedback3) = update_tree (RESET,[MIXED],[id]) tree''' 
		config'''
	...
	let (tree5,config5, feedback4) = update_tree (RESET,[SOFT],[id]) tree''' 
		config'''
	...
	let (tree6,config6, feedback5) = update_tree (RESET,[HARD],[id]) tree''' 
		config'''
...
TEST_MODULE "checkout file" = struct
	...
	let (tree',config',feedback) = init tree config
	let () = Out_channel.write_all "./test_proj/add1.txt" ~data:"Your text"
	let (tree'',config'',feedback') = update_tree (ADD,[EMPTY],["add1.txt"]) tree' 
		config'
	let (tree''',config''',feedback'') = update_tree (COMMIT,[MSG],["add1.txt"]) 
		tree'' config''
	let () = Out_channel.write_all "./test_proj/add1.txt" ~data:"Your text123"
	TEST_UNIT "checkout file" =
		let (_,_,f) = update_tree (CHECKOUT,[FILE],["add1.txt"]) tree''' config''' 
	in
...
TEST_MODULE "branch + checkout tests" = struct
	...
	let (tree''',config''',feedback'') = update_tree (COMMIT,[MSG],["add1.txt"]) 
		tree'' config''
	let (tree4, config4, feedback3) = update_tree (BRANCH,[EMPTY],["hello"]) 
		tree''' config'''
	...
	let (tree5,config5,feedback4) = update_tree (ADD,[EMPTY],["add2.txt"]) 
		tree4 config4
	let (tree6,config6,feedback5) = update_tree (COMMIT,[MSG],["add2.txt"]) 
		tree5 config5
	let (tree7,config7,feedback6) = update_tree (CHECKOUT,[EMPTY],["hello"]) 
		tree6 config6
	TEST_UNIT "checkout" =
...
TEST_MODULE "CONFIG" = struct
	let (tree,config) = setup_tree ()
	let (tree',config',feedback) = init tree config
	let (tree2,config2,feedback1) = 
		update_tree (CONFIG,[CONFIG_SET],["username";"rho"]) tree' config'
	let (tree3,config3,feedback2) = 
		update_tree (CONFIG,[CONFIG_SET],["password";"rho"]) tree2 config2
	let (tree4,config4,feedback3) = 
		update_tree (CONFIG,[CONFIG_SET],["upstream";"vagrant@127.0.0.1"]) tree3 config3
	TEST_UNIT "username" =
...
TEST_MODULE "PUSH AND PULL" = struct
	...
	let (tree''',config''',feedback'') = update_tree (COMMIT,[MSG],["add1.txt"]) 
		tree'' config''
	let (tree4,config4,feedback3) = 
		update_tree (CONFIG,[CONFIG_SET],["upstream";"vagrant@127.0.0.1"]) 
			tree''' config'''
	let (tree4',config4',feedback3) = 
		update_tree (PUSH,[EMPTY],[]) tree4 config4
	TEST_UNIT "push" =
		assert(tree4'=tree4);
